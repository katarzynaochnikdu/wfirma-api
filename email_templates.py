...
# ---------------------------------------------------------------------------
# SZABLON: Przypomnienie o płatności proformy
# ---------------------------------------------------------------------------

TEMPLATE_PROFORMA_REMINDER = '''<!doctype html>
<html lang="pl">
<head>
  <meta charset="UTF-8" />
  <title>Przypomnienie o płatności – {event_name}</title>
  <style type="text/css">
    p, h1, h2, h3, h4, h5, h6, ul {{margin: 0;}}
    @media screen and (max-width: 620px) {{
      .wrapper {{ padding: 8px !important; }}
...
<body>
  <div style="display: none; max-height: 0; overflow: hidden;">
    Przypomnienie o płatności pro-formy – {event_name}
  </div>
                ...
                <tr>
                  <td valign="top" style="padding: 12px 24px 24px 24px; background-color: #FFFFFF;">
                    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="min-width:100%!important;">
                      ...
                      <tr>
                        <td style="padding: 0;">
                          <p style="font-size: 18px; color: #9A3412; font-weight: bold;">⏳ Przypomnienie o płatności pro-formy</p>
                        </td>
                      </tr>
                      <tr><td style="height: 12px;"></td></tr>
                      <tr>
                        <td style="padding: 0;">
                          <p>Przypominamy o płatności pro-formy<strong>{proforma_number_inline}</strong> za udział w wydarzeniu <strong>{event_name}</strong>.</p>
                          {payment_due_date_section}
                        </td>
                      </tr>
                      <tr><td style="height: 16px;"></td></tr>
...
def render_proforma_reminder_email(
    event_name: str,
    purchaser_first_name: str,
    purchaser_last_name: str,
    purchaser_email: str,
    ...
    """
    Renderuje email z przypomnieniem o płatności proformy.
    """
    event_config = event_config or get_default_event_config()
    tickets = tickets or []
    ...
    if payment_due_date:
        payment_due_date_section = f'''
                          <p style="margin-top: 12px; padding: 12px; background: #FEF3C7; border: 1px solid #FDE68A; border-radius: 8px; font-size: 14px; color: #92400E;">
                            <strong>⏱️ Termin płatności:</strong> {payment_due_date}
                          </p>'''
    ...
    return TEMPLATE_PROFORMA_REMINDER.format(**data)
...
# ---------------------------------------------------------------------------
# SZABLON: Potwierdzenie płatności proformy
# ---------------------------------------------------------------------------

TEMPLATE_PROFORMA_PAID_CONFIRMATION = '''<!doctype html>
<html lang="pl">
<head>
  <meta charset="UTF-8" />
  <title>Potwierdzenie płatności proformy – {event_name}</title>
  <style type="text/css">
    p, h1, h2, h3, h4, h5, h6, ul {{margin: 0;}}
    @media screen and (max-width: 620px) {{
      .wrapper {{ padding: 8px !important; }}
...
<body>
  <div style="display: none; max-height: 0; overflow: hidden;">
    Dziękujemy za opłacenie proformy – {event_name}
  </div>
                ...
                <tr>
                  <td valign="top" style="padding: 12px 24px 24px 24px; background-color: #FFFFFF;">
                    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="min-width:100%!important;">
                      ...
                      <tr>
                        <td style="padding: 0;">
                          <p style="font-size: 18px; color: #2E7D32; font-weight: bold;">✅ Dziękujemy za opłacenie pro-formy!</p>
                        </td>
                      </tr>
                      <tr><td style="height: 12px;"></td></tr>
                      <tr>
                        <td style="padding: 0;">
                          <p>Potwierdzamy otrzymanie płatności za pro-formę dotyczącą wydarzenia <strong>{event_name}</strong>.</p>
                        </td>
                      </tr>
                      <tr><td style="height: 24px;"></td></tr>
...
def render_proforma_paid_confirmation_email(
    event_name: str,
    purchaser_first_name: str,
    purchaser_last_name: str,
    purchaser_email: str,
    ...
    """
    Renderuje email z potwierdzeniem płatności proformy.
    """
    event_config = event_config or get_default_event_config()
    tickets = tickets or []
    ...
    data = {
        ...
        "purchaser_email": purchaser_email,
        "purchaser_phone": purchaser_phone or "",
        "tickets_rows": generate_tickets_table_rows(normalized_tickets, event_config.get("color_gradient_1", "#2563eb"), show_summary=False),
        "total_net_formatted": format_currency(vat_calc["net"]),
        "total_vat_formatted": format_currency(vat_calc["vat"]),
        "total_gross_formatted": format_currency(total_gross),
        "event_datetime_section": _build_event_datetime_section(event_config),
        "event_location_section": _build_event_location_section(event_config),
        "color_gradient_1": event_config.get("color_gradient_1", "#2563eb"),
        "color_gradient_2": event_config.get("color_gradient_2", "#1e40af"),
    ...
    return TEMPLATE_PROFORMA_PAID_CONFIRMATION.format(**data)
